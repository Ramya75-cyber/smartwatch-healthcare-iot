# Smartwatch for Healthcare Monitoring

This project is an IoT-based smartwatch designed to monitor health parameters such as heart rate in real-time. It helps elderly people and heart patients by providing continuous health tracking and alerts.

## Features
- Real-time heart rate monitoring
- IoT-based data transmission
- Emergency alerts
- User-friendly interface

## Technologies Used
- Arduino / Embedded System
- IoT Sensors
- Python / Web Dashboard

## Outcome
This project helps improve patient safety by enabling continuous health monitoring.
## Project Overview
This project is an IoT-based smartwatch designed to monitor health parameters such as heart rate in real-time.

## Features
- Real-time heart rate monitoring
- IoT-based data transmission
- Emergency alerts system
- User-friendly dashboard

## Technologies Used
- Arduino / Embedded System
- IoT Sensors
- Python
- Web Dashboard

## Applications
- Healthcare monitoring
- Elderly care
- Heart patient tracking

## Future Enhancements
- Mobile app integration
- AI-based health prediction
